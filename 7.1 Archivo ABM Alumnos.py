    ...
    if t==0:
        ...
        if leg != 0:
            ...
            if pos == -1:               #si no se encontró el Alumno
                ...
                if rta == "S":
                    comint = input("Nueva comisión: ")
                    while validaRangoEntero(comint, 1, 5):
                        comint = int(input("Incorrecto - numero de comision entre 1 y 5: "))

                    RegAlu.comision = str(comint).ljust(2, " ")#lo convierto a string y lo completo para que el registro siga midiendo lo mismo
...
def ListaxPromeDescendente():
    ...
    if t==0:
        ...
        ordenaAlumnosxProme()
        print('legajo  Nombre                     comisión   Carrera  nota1  nota2  nota3   Promedio Baja')
        print('--------------------------------------------------------------------------------------')
        ArcLogAlu.seek(0, 0)
        RegAlu = Alumno()
        while ArcLogAlu.tell()<t:
